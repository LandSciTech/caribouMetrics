...
getScenarioDefaults <- function(paramTable = NULL,
  ...
  if (hasName(paramTable,"cowMult") & any(paramTable$cowMult != 1) & is.element("cowCount", names(paramTable))) {
  ...
  if(hasName(paramTable,"cowCount") && sum(paramTable$collarCount>paramTable$N0)>0){
  ...
  if (!hasName(paramTable,"startYear")) {
