...
#'   confidence interval.
#'
#' @return For `getNationalCoefficients` a list with elements:
...
getNationalCoefficients <- function(replicates,
  ...
  DT_S <- subsetNationalCoefs(populationGrowthTable,
                    resVar = "femaleSurvival",
                    modelVersion = modelVersion,
                    modNum = survivalModelNumber)[[1]]

  coefSamples_S <- sampleNationalCoefs(DT_S, replicates)

  DT_R <- subsetNationalCoefs(populationGrowthTable,
                    resVar = "recruitment",
                    modelVersion = modelVersion,
                    modNum = recruitmentModelNumber)[[1]]

  coefSamples_R <- sampleNationalCoefs(DT_R, replicates)
