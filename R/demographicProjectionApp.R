...
$( document ).ready(function() {
  ...
  ui <- dashboardPage(
    ...
    dashboardSidebar(
      ...
      shinyFiles::shinySaveButton("saveOutput", title = "Save results to csv or rds",
  ...
  server <- function(input, output, session) {
    ...
    observeEvent(c(input$scn_file, input$saveOutput, input$popGrowth_file), {
      shinyFiles::shinyFileChoose(input, "scn_file", roots = volumes(), session = session,
                                  filetypes = "csv")

      shinyFiles::shinyFileChoose(input, "popGrowth_file", roots = volumes(), session = session,
                                  filetypes = "csv")

      shinyFiles::shinyFileSave(input, "saveOutput", roots = volumes(), session = session,
                                filetypes = c("csv", "rds"))
    })
    ...
    output$plot6 <- renderPlot({
      ...
      dist %>%
        tidyr::pivot_longer(c(Anthro, fire_excl_anthro), names_to = "dist_type",
                     values_to = "dist") %>%
    ...
    observeEvent(input$saveOutput, {
      if(!is.integer(input$saveOutput)){
        ...
        if(grepl("\\.csv$", path)){
          ...
          warning("file ", path, " could not be saved. Ensure that the file",
