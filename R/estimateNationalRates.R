...
#'
#' @description Apply the sampled coefficients to the disturbance covariates
#'   to calculate expected recruitment and survival according to the beta
#'   regression models estimated by Johnson et al. (2020).`estimateNationalRates` is a wrapper
#'   around `estimateNationalRate` to sample both survival and recruitment rates based on
#'   the result of [getNationalCoefficients()] and using recommended defaults.
#'
#' @details  Each population is optionally assigned to quantiles of the Beta
#'   error distributions for survival and recruitment. Using quantiles means
#'   that the population will stay in these quantiles as disturbance changes
#'   over time, so there is persistent variation in recruitment and survival
#'   among example populations.
#'
#'   A transformation function is also applied to survival to avoid survival
#'   probabilities of 1.
#'   
#'   A detailed description of the model is available in [Hughes et al. (2025)](https://doi.org/10.1016/j.ecoinf.2025.103095)
#'   
#'
...
#'   returned by `getNationalCoefficients`.
...
#' @family demography
#' @export
estimateNationalRates <- function(covTable,
                             popGrowthPars,
  ...
  pred_S <- estimateNationalRate(covTable = covTable,
  ...
  pred_R <- estimateNationalRate(covTable = covTable,
