...
if(file.exists(mod_fl)){
  ...
  mod <- bayesianTrajectoryWorkflow(surv_data = bboudata::bbousurv_a %>% filter(Year > 2010), 
...
mod_tab <- suppressWarnings(compareTrajectories(mod))
...
test_that("all parameters work",{
  plotCompareTrajectories(mod_tab, param_nms) %>% expect_type("list")
})
