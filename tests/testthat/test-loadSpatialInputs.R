...
landCover <- terra::rast(file.path(pth_base, "landCover.tif")) 
...
anthroDist <- terra::rast(file.path(pth_base, "anthroDist.tif"))

# make a polygon inside the bounding box of the rasters
singlePoly <- (terra::ext(landCover) - 30000) %>% st_bbox() %>%
...
linFeat400 <- terra::rast(file.path(pth_base, "linFeatTif.tif")) %>% 
  terra::resample(y = tmplt, method = "bilinear")
...
out2 <- loadSpatialInputs(
  ...
  reclassOptions = list(natDist = list(fn = reclassDist,
                                       endYr = 2020, 
                                       numCumYrs = 30,
                                       dateField = "FIRE_YEAR"))
)
...
test_that("works when inputs have different crs", {
  ...
  expect_s4_class(out3$refRast, "SpatRaster")
})
