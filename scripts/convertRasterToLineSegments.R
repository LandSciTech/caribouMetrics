...
#' @export
convertRasterToLineSegments<-function(cMap, useMST=F){
  ...
  #This doesn't work for large problems. Try using road package functionality.
  if(!useMST){
    ...
    #cMap=crop(cMap,extent(cMap,1, 100, 1, 100))
    #plot(cMap)
    #TO DO: revise roads package to make this work.
    cost=cMap;cost[cost>0]=0;cost[cMap==0]=100
    
    
    rds = raster::rasterToPoints(cMap, fun = function(x){x > 0},
                                 spatial = TRUE) 
    rds=spsample(rds,n=,type="random")
    
    
    nout = roads::projectRoadsNew(landings = cMap,
                                  cost = cost,
