...
The second step in `updatePLC` is to convert the PLC data to a 16 ha resolution by layerizing and then using bilinear interpolation so the value in the 16 ha raster is the proportion of that cell covered by the resource type. If > 35%  of a cell is disturbed by natural disturbance then values of resource types other than DTN and LGW are set to 0. If > 35% was disturbed by either anthropogenic disturbance or harvest then the values of resource types other than DTN, LGW, LGTP, LGOP and ST are set to 0.
...
toUpdate <- ((age > 35) & (natDist == 1)) | harv == 1 | plc == ATNcode
...
for (i in toChange) {
  plc[[i]] <- raster::mask(plc[[i]], allDist16ha[[3]] > 0.35,
                           maskvalue = 1, 
                           updatevalue = 0)
}
...
for (i in toChange) {
  plc[[i]] <- raster::mask(plc[[i]], max(allDist16ha[[1:2]] > 0.35),
                           maskvalue = 1, 
                           updatevalue = 0)
}
