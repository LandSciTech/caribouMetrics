...
* [Horseth and Rempel 2016](https://doi.org/10.1139/cjz-2015-0101)
...
plc = inData@landCover; fri = inData@updatedLC; age = inData@age; 
natDist = natDistD; anthroDist = anthroDistD;
harv = harvD; resTypeLevels = resTypeCode
...
tmapCatRast(plc, resTypeLevels$ResourceType, "To Update PLC data")
...
tmapCatRast(plc, resTypeLevels$ResourceType, "Post-update PLC to FRI data")
...
plc <- raster::layerize(plc) %>% 
  ...
  `names<-`(resTypeLevels$ResourceType)
...
toChange <- resTypeLevels %>% filter(!ResourceType %in% c("DTN", "LGW")) %>% 
  pull(code)
...
qtm(plc, title = "PLC data after 0 > 35% natDist or anthroDist")
