...
`estimateBayesianRates()` returns fitted bboutools survival and recruitment models (assuming no trends over time) and summaries of those models. In this example, we add 5 missing years that will be predicted by the model. 
```{r fit bbou model, results='hide'}
...
```{r explore bbou model}
names(mod)
mod$parTab
```

Use `simulateTrajectoriesFromPosterior()` to predict demographic trajectories from the mcmc samples. Use `summarizeTrajectories()` to get 95% prediction intervals from the demographic trajectories. 
...
```{r plot1, fig.width=8, fig.height=10, fig.cap="Example trajectories and 95% prediction intervals from mcmc samples."}
...
```{r plot2, fig.width=8, fig.height=10, fig.cap="Comparison of 95% prediction intervals from mcmc samples (grey ribbon) with results returned by bboutools (points and error bars)."}
...
The model summary returned by `estimateBayesianRates()` can also be used to simulate demographic trajectories. This simulates trajectories using the modeled mean and standard deviation of demographic rates and their interannual variation across the years in the data. Therefore the distribution across years stays similar. **TODO is there any difference or benefit to creating projections for the future by adding missing years before fitting the bboutools model vs using trajectoriesFromSummary?**
...
```{r plot3, fig.width=8,fig.height=10, fig.cap="Example trajectories and 95% prediction intervals from the model summary."}
...
```{r plot4, fig.width=8, fig.height = 10, fig.cap="Comparison of 95% prediction intervals derived from the model summary with results returned by bboutools."}
...
```{r plot5, fig.width=8, fig.height= 8, fig.cap="Comparison of 95% prediction intervals derived from the model summary with a scenario in which expected calf:cow ratio increases by 30%."}
