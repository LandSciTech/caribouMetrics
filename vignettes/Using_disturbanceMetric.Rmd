...
# `disturbanceMetrics()`
The `disturbanceMetrics()` function is used to calculate the metrics described in Table 52 of Environment Canada Scientific Assessment to Inform the Identification of Critical Habitat for Woodland Caribou (*Rangifer tarandus caribou*), Boreal Population, in Canada [@eccc_scientific_2011]. The metrics included are:
...
`disturbanceMetrics` will prepare the data and then calculate disturbance metrics used as predictor variables by @johnson_science_2020. The function can be run in several different ways, and the simplest is to provide spatial objects for each input. 
