...
```{r disturbanceMetricsRaster, results='hide',message=F, warning=F}
...
```
Multiple linear feature inputs can also be provided in vector form. See `vignette("caribouHabitat.Rmd", package = "caribouMetrics")` for other data processing and input options. 

```{r disturbanceMetricsVector, results='hide',message=F, warning=F}
...
A simple case for demographic projection is multiple stochastic projections from a single landscape that does not change over time. We begin by sample coefficients for 500 replicate populations using default models M1 and M4. Next we calculate sample demographic rates given sampled model coefficients and disturbance metrics for our example landscape, setting `returnSample = TRUE` so that the results returned contain a row for each sample in each scenario. We set an the initial population size for each sample population to 100, and project population dynamics for 20 years using the `popGrowthJohnson` function with default parameter values. Anthropogenic disturbance is high on this example landscape, so projected population growth rate for most sample populations is below 1, but uncertainty in the model means that a few sample populations persist.


```{r simpleProjection, results='hide',message=F, warning=F}
   ...
   rateSamples <- demographicRates(
          ...
          useQuantiles = FALSE)     
   rateSamples$N0=100
   demography <- cbind(rateSamples,popGrowthJohnson(N=rateSamples$N0,numSteps=20,
                                                    Rec_bar=rateSamples$R_bar,
                                                    S_bar=rateSamples$S_bar))

```
```{r plotSimpleDemography, fig.width=8, fig.height=5}

fds <- pivot_longer(demography,!(scnID:replicate)&!N0,names_to="Metric",values_to="Amount")
d1 <- ggplot(fds,aes(x=as.factor(round(Anthro,2)),y=Amount,colour=fire_excl_anthro))+geom_violin(alpha=0.4,color="black")+
  geom_point(shape = 21,size=2, position = position_jitterdodge())+
  facet_wrap(~Metric,scales="free")+theme(legend.position = "none")+xlab("Anthro")
...
```{r disturbanceCovariates, results='hide',message=F, warning=F}
...
```{r popGrowthParameters, results='hide',message=F, warning=F}
...
```
### No precision
Johnson 2020 model with parameter uncertainty but no precision. The bands are the 2.5% and 97.5% quantiles of 500 sample parameter values.
...
# at each time,  sample demographic rates and project, save results
  ...
  for(t in 1:numTimesteps){
    ...
    rateSamples <- demographicRates(covTable = covariates,
                                  popGrowthPars = popGrowthParsSmall,
                                  ignorePrecision = FALSE,
                                  returnSample = TRUE,
                                  useQuantiles = TRUE)
    ...
    pars = cbind(pars,popGrowthJohnson(pars$N0,Rec_bar=pars$R_bar,S_bar=pars$S_bar,
                                     numSteps=stepLength,interannualVar=F,Kmultiplier=F,
...
base1 <- ggplot(data = popMetrics, aes(x = Timestep,y=Amount,colour=Replicate,group=Replicate))+
  geom_line()+facet_wrap(~MetricTypeID,scales="free")+
