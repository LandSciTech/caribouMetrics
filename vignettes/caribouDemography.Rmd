...
Interannual variation in survival and recruitment is modelled using truncated beta distributions [rtrunc function; Novometsky et al. 2016](https://CRAN.R-project.org/package=truncdist): $\dot{R}_t \sim \text{TruncatedBeta}(\bar{R}_t,\nu_R,l_R,h_R); \dot{S}_t \sim \text{TruncatedBeta}(\bar{S}_t,\nu_S,l_S,h_S)$. $\nu_R$ and $\nu_S$ are coefficients of variation
among years and $l_R,h_R,l_S$, and $h_S$ are maximum/minimum values for recruitment and survival.
