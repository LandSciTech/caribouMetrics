...
vignette: >
  %\VignetteIndexEntry{Caribou RSPF package}
...
## Simple Example
The example data set loaded below inlcudes a small area in the Churchill caribou range that we will use as an example. 
...
`caribouHabitat` is the primary function in the package. It will prepare the data, process it into the explanatory variables in the Hornseth and Rempel RSPF models and then calculate the probability of habitat use in each season at each location. The function can be run in several different was but the simplest is to provide spatial objects for each dataset. Other required arguments for the function are a lookup table that converts the forest units used in the FRI data to the regional forest units (RFUs) used in the model, and the name of the caribou range that the project is located in.
...
The `caribouHabitat` function returns an S4 object with the class `CaribouHabitat` which has slots for the input data, the hexagonal grid, the processed data, and the results. To access the results as a RasterStack object that can be used for ploting or graphing use the `results` function.

```{r results1}
carHabRes1 <- results(carHab1)
carHabRes1
```
...
```{r create projPoly}
# interactively you can do the following
# plot(plcD)
# ext <- raster::drawExtent()
# 

# We will hard code this to run non-interactively
...
carHab2 <- caribouHabitat(
  ...
  friLU = read.csv(paste0(pthBase, "friLU", ".csv"), stringsAsFactors = FALSE), 
  caribouRange = "Churchill", padProjPoly = TRUE, winArea = 1000
)
```

```{r inspect padded}
plot(plcD, col = "black", legend = FALSE)
plot(carHab2@plc, add = TRUE)
plot(carHab2@projectPoly %>% st_geometry(), add = TRUE)
```

The black area on the map above shows the extent of the original plc data, the raster shows the proportion of conifer in the processed plc data that has been cropped to within 1750 m of the projectPoly which is the black rectangle.  
...
carHab3 <- caribouHabitat(
  ...
  linFeat = linFeatDras, projectPoly = projectPolyD2,
  friLU = read.csv(paste0(pthBase, "friLU", ".csv"), stringsAsFactors = FALSE), 
  caribouRange = "Churchill", padProjPoly = FALSE, winArea = 1000
)
st_area(projectPolyD2)

tmap::qtm(carHab3@projectPoly, fill = NULL)+
plot(carHab2, season = "Fall", title = "With padding", 
     raster.breaks = c(0, 0.025, 0.05, 0.1, 0.15, 0.2, 0.3, 0.4, 0.5, 1), 
     layout.legend.outside = TRUE)
tmap::qtm(carHab3@projectPoly, fill = NULL)+
plot(carHab3, season = "Fall", title = "Without padding", 
     raster.breaks = c(0, 0.025, 0.05, 0.1, 0.15, 0.2, 0.3, 0.4, 0.5, 1),
     layout.legend.outside = TRUE)
```

The difference is very large in this example because the project area is very small compared to the window size. The version with padding shows the highest probability of use in the top left corner because of the large conifer dominated (pink) area in the top left. Because this conifer dominated area is mostly outside the projectPoly it is not picked up in the version without padding and as a result the probability of use is lower. Padding also prevents the loss of information due to edge cells having NAs. 

## Example with other inputs
As mentioned above `caribouHabitat` can be called in several different ways. You can provide the data as filepaths instead of spatial objects, you can provide a previously created hexgrid to save time making it again, you can provide the eskers and linear features as vector files which will be updated, and you can provide roads, rail, and utilities as a list to be combined into the linear features. 
...
carHabLF1 <- caribouHabitat(
  plc = plcD, esker = eskerDras, fri = friD, age = ageD, natDist = natDistD, 
  anthroDist = anthroDistD, harv = harvD,
  linFeat = linFeatDras1, projectPoly = projectPolyD,
  hexgrid = hexgridD,
  friLU = read.csv(paste0(pthBase, "friLU", ".csv"), stringsAsFactors = FALSE), 
  caribouRange = "Churchill"
)
...
carHabLF2 <- updateCaribou(
  carHabLF1, newData = list(linFeat = linFeatDras2),
  friLU = read.csv(paste0(pthBase, "friLU", ".csv"), stringsAsFactors = FALSE), 
  caribouRange = "Churchill"
)

# compare results
plot(carHabLF1, season = "Fall", title = "linFeat 1", raster.breaks = 0:12/2/10)
plot(carHabLF2, season = "Fall", title = "linFeat 2", raster.breaks = 0:12/2/10)
...
purrr::walk2(c(carHabLF1, linFeatResults), 
             ...
             ~plot(.x, season = "Fall", main = .y, breaks = 0:12/2/10) %>% print())
