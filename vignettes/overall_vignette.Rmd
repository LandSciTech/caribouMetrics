...
## Simple Example
The example data set loaded below includes a small area in the Churchill caribou range that we will use as an example. 
...
carHab4 <- caribouHabitat(
  ...
  friLU = read.csv(paste0(pthBase, "friLU", ".csv"), stringsAsFactors = FALSE) %>%
    mutate(RFU = toupper(RFU) %>% stringr::str_replace("HRDMW", "HRDMX")), 
  caribouRange = "Churchill", padProjPoly = FALSE, padFocal = TRUE,
  winArea = 500
)
