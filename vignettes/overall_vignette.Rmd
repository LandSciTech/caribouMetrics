...
`caribouHabitat` is the primary function in the package. It will prepare the data, process it into the explanatory variables in the Hornseth and Rempel RSPF models and then calculate the probability of habitat use in each season at each location. The function can be run in several different was but the simplest is to provide spatial objects for each dataset. Other required arguments for the function are a lookup table that converts the forest units used in the FRI data to the regional forest units (RFUs) used in the model, and the name of the caribou range that the project is located in. In this case we also supply the area of the window used to calculate a moving window average of the input variables, this is only for the purpose of keeping the example data small and should not be supplied in most cases. 
...
```{r caribouHabitat2}
# Use dummyD so that plc is not updated and is the main source of habitat info
dummyD <- raster::setValues(plcD, NA)

carHab2 <- caribouHabitat(
  plc = plcD, esker = eskerDshp, fri = dummyD, age = dummyD, natDist = dummyD, 
  anthroDist = dummyD, harv =dummyD,
  linFeat = linFeatDshp, projectPoly = projectPolyD2,
  friLU = read.csv(paste0(pthBase, "friLU", ".csv"), stringsAsFactors = FALSE) %>%
  mutate(RFU = toupper(RFU) %>% stringr::str_replace("HRDMW", "HRDMX")),
...
tmap::qtm(plcD, raster = "black")+
  tmapCatRast(carHab2@plc, lbls = raster::levels(carHab2@plc)[[1]][[2]], ttle = "PLC")+
...
The difference is large in this example because the project area is very small compared to the window size. Based on the PLC data in the previous map you can see that there is one DEC cell just outside the projectPoly on the right. DEC has a strong negative effect on caribou habitat use so in the version with padding cells on the right edge have lower probability of use than in the version without padding. In the top left corner the values in the version without padding are lower than with padding because the non padded version treats the cells outside the projectPoly as 0 when taking the weighted sum of values in the window. Therefore it is highly recommended to provide data for the area around the project and to use padProjPoly = TRUE.  
