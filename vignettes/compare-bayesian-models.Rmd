...
@hughes_integration_2025 described a Beta model with disturbance covariates and informative priors for analysis of a single boreal caribou population with survival data aggregated by year and only cows and calves reported in the composition survey. To align with and allow comparison to the models implemented in bboutools [@dalgarno_bbousuite_2025] we extended the @hughes_integration_2025 models as follows:
...
-   Allow for analysis of groups of populations with shared interannual variation. See [Multi-Population Analysis and Other Extensions](https://poissonconsulting.github.io/bboutools/articles/extensions.html) for details.
...
surv_dataNone <- surv_data %>% getCaribouYear() %>% filter(CaribouYear>2017)
recruit_dataNone <- recruit_data %>% getCaribouYear() %>%  filter(CaribouYear>2017)
...
As shown by @hughes_integration_2025 the prior means and 95% prior predictive intervals from the Beta model are similar to the means and ranges between the 2.5% and 97.5% quantiles of 3000 simulated survival and recruitment trajectories from the national model (Figure \@ref(fig:fig-plot1)).
...
```{r compare beta and bbou priors, results='hide'}
