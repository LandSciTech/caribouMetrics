...
```{r}
last_observed_year <- 2016 #historical baseline cutoff (last year of observed data)
future_years <- 2017:2021 #future years to simulate

simLimitedCorr <- trajectoriesFromBayesian(betaLimited, 
                                           cPars = list(correlateRates = TRUE))

surv <- surv_dataLimited %>%
  mutate(Year = Year) %>%
  filter(!is.na(Year)) %>%
...
#Extract cow counts
rec_simple <- recruit_dataLimited %>%
  mutate(Year = Year) %>%
  filter(!is.na(Cows)) %>%
